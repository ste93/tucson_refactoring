...
//				tid = new TucsonTupleCentreId(name + "@'" + address + "'");
			...
			{
				stateBar.setText("Operation Failed: " + ex);
				ex.printStackTrace();
			}
			// Add your handling code here:
		}
	...
	{// GEN-FIRST:event_buttonNextStepActionPerformed
		try
		{
			context.nextStep();
		}
	...
	{// GEN-FIRST:event_checkStepExecutionActionPerformed
		try
		{
	...
	}// GEN-LAST:event_checkStepExecutionActionPerformed
	...
	private void inputFreqActionPerformed(java.awt.event.ActionEvent evt)
	{// GEN-FIRST:event_inputFreqActionPerformed
	}// GEN-LAST:event_inputFreqActionPerformed
...
//		TucsonAgentLanguageManager.useLanguage(TucsonLanguageManager.DEFLANGUAGE);
		...
		if (alice.util.Tools.isOpt(args, "-?"))
		{
			...
			System.out.println("{-tid tuple centre identifier} {-aid agent identifier} {-?}");
			System.exit(0);
		}
		...
		else
		{
			try
			{
				st_aid = "inspector_" + System.currentTimeMillis();// + "@" + java.net.InetAddress.getLocalHost().getHostAddress();
			}
			catch (Exception ex)
			{
				st_aid = "inspector_" + System.currentTimeMillis();
			}
		}
		
		String st_tid = null;
		
		if (alice.util.Tools.isOpt(args, "-tid"))
		{
			st_tid = alice.util.Tools.getOpt(args, "-tid");
		}

		System.out.println("Agent Identifier: " + st_aid);
		
		if (st_tid != null)
		{
			System.out.println("Tuple Centre Identifier: " + st_tid);
		}
		...
		try
		{
			...
			if (st_tid != null)
			{
				tid = new TucsonTupleCentreId(st_tid);
			}
		}
		catch (Exception ex)
		{
			System.err.println("INSPECTOR FAILED: " + ex);
			System.exit(-1);
		}
		
		System.out.println("INSPECTOR BOOTED.");
		...
		try
		{
			...
			System.out.println("INSPECTOR QUIT.");
			System.exit(0);
		}
		catch (Exception ex)
		{
