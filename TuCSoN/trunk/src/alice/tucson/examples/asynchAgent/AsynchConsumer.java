...
public class AsynchConsumer extends AbstractTucsonAgent {
	...
	public AsynchConsumer(String id, int nOperations)
		...
		public void operationCompleted(AbstractTupleCentreOperation op) {
			...
			if (op.isResultSuccess()) {
				...
				try {
					...
					addColor(color);
					log("[consumer listener]green=" + green + " blue=" + blue
	...
	protected void main() {
		try {
			...
			System.out.println("In number:"
					+ aqm.getCompletedQueue().getAllTypedOperation(In.class)
							.size());
			...
			aqm.shutdownNow();
			log("[" + this.getTucsonAgentId() + "]: green=" + green + " blue="
					+ blue + " yellow=" + yellow + " red=" + red + " e count="
					+ count + " e failedFOrTime:" + failedForTime);
	...
	public static void main(final String[] args) {
		/*
		 * String aid = null; if (args.length == 1) { aid = args[0]; } else {
		 * aid = "asynqqueue"; } try { new AsynchConsumer("consumer",
		 * 1000).go(); } catch (TucsonInvalidAgentIdException e) { // TODO
